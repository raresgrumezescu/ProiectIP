...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///IPproiect.db'
...
def signup():
	if request.method=="POST":
		...
		if(password!=repassword):
			return render_template("error.html", message="Parolele nu se potrivesc!")
		...
		try:
			...
			return render_template("error.html", message="Un User cu acelasi nume exista deja!")
		return render_template("login.html", msg="Cont creat!")
...
def login():
	if request.method=="POST":
		...
		if result == None or not check_password_hash(result.password, password):
			return render_template("error.html", message="User si/sau parola invalide")
...
def home():
	if request.method == "POST":
		...
		return render_template("home.html", rows=rows, message="Produs adaugat")
...
def edit(pro_id):
	...
	if request.method == "POST":
		...
		if result.username != session['username']:
			return render_template("error.html", message="Nu esti autorizat sa modifici acest produs!")
		category= request.form.get("category")
		name = request.form.get("pro_name")
		description = request.form.get("description")
		price_range = request.form.get("price_range")
		...
		db.session.commit()
		rows = Product.query.filter_by(username=session['username'])
		return render_template("home.html", rows=rows, message="Produs editat")
